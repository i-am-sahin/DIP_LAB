%9. Write a program to performs discrete wavelet transform on image.
%=>
% Step 1: Read the image from MATLAB library
img = imread('cameraman.tif');
% Step 2: Convert to grayscale if it is not already
if size(img, 3) == 3
   img = rgb2gray(img);
end
% Step 3: Define Haar wavelet filters
Lo_D = [1 1]/sqrt(2); % Low-pass filter for decomposition
Hi_D = [-1 1]/sqrt(2); % High-pass filter for decomposition
% Step 4: Perform single level 2D DWT manually
